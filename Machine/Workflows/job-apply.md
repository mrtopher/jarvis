...
2. **Recruiter-audit loop (must reach >= 90/100).** Adopt the recruiter persona below and audit the current resume against the target JD. **Score blind and cold to avoid anchoring:** derive the Phase 2 number purely from this resume against this JD before looking at any prior application's score, and never reuse a previous score as a target or landing spot. A cluster of identical scores across applications (e.g. several 91/pass-1 in a row) is a red flag that you are anchoring, not auditing. The score must move with real fit: a genuine experience gap (scale, domain, headcount) that cannot be honestly written around should hold the resume *below* 90 and be reported as such, not nudged over the bar. Be willing to score in the 80s. Loop:
...
4. **Accuracy and honesty pass (always run, remediate in place).** Before the voice pass, audit every line of the resume YAML (and the cover letter YAML, if present) for factual truth and honest self-representation. This pass is non-negotiable - a resume that scores 90 on impact can still contain a false claim, so run it every time. Flag and fix:
